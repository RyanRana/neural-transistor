...
def prune_by_reliability(ir: CircuitIR, p_real: float, model=None) -> tuple[CircuitIR, dict]:
    ...
    flyforge.circuit.noise for how the curve is measured.
    """
    from flyforge.circuit import noise as _noise
    model = model or _noise.NoiseModel.load()
    if model is None:
        raise RuntimeError(
            "no noise model cached; run flyforge.circuit.noise.measure(conn) once")
