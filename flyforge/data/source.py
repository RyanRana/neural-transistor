...
_DEFAULT_CACHE = Path.home() / ".cache" / "flyforge"
...
def data_dir() -> Path:
    return Path(os.environ.get("FLYFORGE_DATA", _DEFAULT_DATA))


def cache_dir() -> Path:
    d = Path(os.environ.get("FLYFORGE_CACHE", _DEFAULT_CACHE))
        ...
        for f in (ANNOTATIONS, NEUROTRANSMITTERS, WEIGHTS):
            if not (d / f).exists():
                raise FileNotFoundError(
                    f"{f} not found in {d}. Set FLYFORGE_DATA to the directory "
                    f"holding the male-CNS release."
                )

        def say(m):
            if verbose:
                print(f"[flyforge] {m}", flush=True)
